...
- **Every migration is reversible** — `downgrade()` drops what `upgrade()` adds. Current head: `0013_add_ingestion_runs` (down-rev `0012_phase_c_scan_recency`) — creates the `ingestion_runs` table (one row per cycle attempt) backing the eagle-eye health view (`GET /admin/sources` enrichment + `…/{source}/runs`); `status`/`trigger` CHECKs + `idx_ingestion_runs_source_started (source, started_at DESC)` + partial `idx_ingestion_runs_running WHERE status='running'`; swept after 90 days by `app/core/sweeps.py`. `0012_phase_c_scan_recency` adds the nullable `catalog_items.last_deep_scan_at` + `last_lite_scan_at` recency columns the I-04 Phase C auto-scan triggers read (D-04-14/15); `scan_runs.scan_depth` was intentionally NOT added (the `enqueue_scan` completion wrapper knows the depth and stamps the right column directly). Chain: `0013` ← `0012` (scan recency) ← `0011` (outbox tables) ← `0010` (full projection) ← `0009` (native enums) ← `0008`. Reversible downgrades throughout with best-effort NOT NULL re-assertions (as `0007` does for `github_url`).
...
| New internal (hand-written) model added | "Models are codegen-driven" — extend the eleven-store list + `app/models/__init__.py` docstring; confirm it has no wire DTO (else it must be schema-driven/generated) |
