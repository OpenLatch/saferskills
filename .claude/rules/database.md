...
- **Every migration is reversible** — `downgrade()` drops what `upgrade()` adds. Current head: `0026_add_crawler_alert_cooldown` (down-rev `0025_widen_ingestion_trigger`) — adds two nullable columns to `crawler_cursors` (`last_alerted_at TIMESTAMPTZ`, `alert_signature VARCHAR(100)`) backing the per-source ingestion-alert cooldown (`framework/alerts.py`); durable so a worker restart doesn't reset the throttle and resume the Slack spam. No CHECK/enum; reversible drop. Below it: `0025_widen_ingestion_trigger` (down-rev `0024_skill_compat_all_agents`) — widens `chk_ingestion_runs_trigger` to admit the `reconcile` value the overdue-cycle reconciler writes (PR #129 added the writer but no migration widened the 0013 CHECK, so every reconcile run-record INSERT was silently rejected; see § Ingestion runs). Below it: `0024_skill_compat_all_agents` re-derives the `skill` agent_compatibility set to all eight agents, and `0023_agent_component_scan_link` (down-rev `0022_drop_agent_verify_waitlist`) adds the nullable `agent_runs.component_scan_run_id` FK → `scan_runs(id)` `ON DELETE SET NULL` (DB-only `x-postgresql-extra-columns` on `agent-scan-report.schema.json`; the generated `AgentRun` model carries the `mapped_column`, NOT the wire entity), linking a best-effort CLI-captured component scan (the `scan --local` upload of the scanned platform's installed capabilities) onto the agent run so the Agent Report projects that run's per-capability `scans` into the **Component Scores** tab; NULL on web / `--print-skill` paths (no local filesystem); `SET NULL` (not CASCADE) so deleting the component scan never deletes the behavioral run; lineage …→0021→0022→0023→0024→0025→0026. See § Agent directory surface. The per-revision DDL for the full `0001 → 0026` lineage lives in the migration files themselves (`services/api/migrations/versions/`) — read those / `git log` rather than mirroring the whole history here. Each feature-bearing revision has a dedicated section below: `§ Agent directory surface` (0020–0023), `§ Agent scan` (0019), `§ Install events` (0015), `§ CLI Proof-of-Work spent ledger` (0016), `§ Auto-scan queue-of-record` (0014), `§ Ingestion runs` (0013). Reversible downgrades throughout.
...
### Idempotency cache never serves a `failed` run — re-run in place

The public idempotency cache (`scan_runs.idempotency_key` UNIQUE, `select_existing_run_by_idempotency`, used by `POST /scans` + `POST /scans/upload`) returns a cached run **only** when it is `completed` or still in flight (`pending`/`running`). A **`failed`** idempotency hit is NOT a cache hit: re-submitting the same content must re-attempt the scan, not re-serve the stale failure forever (a transient infra failure since fixed — e.g. the asyncpg-NOTIFY-pool incident left runs cached `failed`, so a public re-upload kept rendering "scan didn't complete"). The submit routers detect a `failed` hit and call `app/scan/persistence.py::reset_failed_run_for_rerun(session, run)`, which **reuses the run id** (so its report URL keeps working) — resetting `status→pending` + score/tier/tally/counts, and clearing the run's children (mirrors `delete_run_cascade`'s child order but KEEPS the `scan_runs` row), so in particular the stale `scan_events` are deleted and the SSE replay re-streams the fresh run from `event_seq` 1. Then it re-dispatches the scan task on the same id. Unlisted runs never cache (per-submission nonce), so this applies only to the public path.
...
| Piece | Shape | Role |
|---|---|---|
| `ingestion_runs` | `id` UUID PK; `source`; `trigger` (chk `scheduled`/`manual`/`force`/`reconcile` — `reconcile` added in `0025_widen_ingestion_trigger`); `status` (chk `running`/`succeeded`/`failed`); `started_at`/`ended_at`/`duration_ms`; `items_seen`/`items_added`/`items_updated`/`http_304_count`/`http_5xx_count`; `attempt`; `error_class`/`error_message` (≤2048 chars); `created_at` | Run history + rolling failure-window source for `GET /admin/sources` + `…/{source}/runs`. |
...
- **Run-level metrics come from here; cursor-level health stays on `crawler_cursors`** (`consecutive_failure_count` + `last_successful_cycle_at`). The pure verdict logic is `app/ingestion/framework/health.py`. The `trigger` is threaded from the call site: periodic → `scheduled`, admin force-cycle → `force`, `run_one_cycle` → `manual`, the overdue-cycle reconciler → `reconcile` (`tasks.defer_source_cycle`). **A new `trigger` value needs a CHECK-widening migration** (the value list is hardcoded in the migration, per `ingestion.md` § Adding a provider) — the omission of `reconcile` from 0013 was the keystone staging bug (every reconcile run-record was rejected → the reconciler saw no attempts → re-fired daily sources every 15 min).
...
| `crawler_cursors` alert-cooldown column change | "Migrations" head note + `app/models/crawler_cursor.py` (`last_alerted_at`/`alert_signature`) + `app/ingestion/framework/alerts.py` + `ingestion.md` § Periodic tasks (Alerts) |
...
| Idempotency cache / `reset_failed_run_for_rerun` change | "Idempotency cache never serves a `failed` run" + `app/scan/persistence.py` + `app/routers/scans.py` (both submit paths) |
