...
| `agent_scan_telemetry` | write-only company-level ASN + fingerprint, no raw IP/PII; `agent_run_id` FK **`SET NULL`** | Anonymous aggregate (reader deferred to I-06). Survives run deletion via `SET NULL`. Hand-written `app/models/agent_scan_telemetry.py`. See `privacy.md` § agent_scan_telemetry. |
...
- **Deletion cascade.** Deleting an `agent_runs` row **CASCADEs** its `agent_findings` + `agent_evidence`, and **SET NULLs** its `agent_scan_telemetry` (so the anonymous aggregate survives). The `agent_run_token_spent` ledger is **NOT cascaded** — its rows die only by expiry sweep.
...
## Agent directory surface (`agent_runs.vendor_reply` + `agent_verify_waitlist`, migration 0020)

I-5.6 Phase C adds the public `/agents` directory + the right-of-reply + the
verify-tier demand signal. Two storage changes:
...
- **`agent_verify_waitlist`** (hand-written internal store, `app/models/agent_verify_waitlist.py`)
  — one row per "Request independent verification" demand signal (D-5.6-08). Columns:
  `id` UUID PK; `email` VARCHAR(320) nullable (account-free, email-OPTIONAL);
  `redacted_ip` VARCHAR(64) nullable (/24-v4 or /48-v6 at write time — never a raw
  IP, `privacy.md`); `created_at`. Index `idx_agent_verify_waitlist_created`.
  **Internal storage only** — NOT part of the generated entity pipeline; registered
  in `app/models/__init__.py`. Rows are **retained** (no PII beyond the opt-in
  email) — the demand signal is the point. Written by
  `POST /agent-scans/verify-waitlist` (rate-limited on the new `agent_verify_waitlist`
  bucket). See `privacy.md` § agent_verify_waitlist.
- **`chk_rate_limits_bucket` gains `agent_verify_waitlist`** (the waitlist-submit
  per-IP cap) — VARCHAR+CHECK, lineage 0001 → 0006 → 0008 → 0016 → 0019 → 0020.
