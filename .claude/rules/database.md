...
- **Every migration is reversible** — `downgrade()` drops what `upgrade()` adds. Current head: `0023_agent_component_scan_link` (down-rev `0022_drop_agent_verify_waitlist`) — adds the nullable `agent_runs.component_scan_run_id` FK → `scan_runs(id)` `ON DELETE SET NULL` (DB-only `x-postgresql-extra-columns` on `agent-scan-report.schema.json`; the generated `AgentRun` model carries the `mapped_column`, NOT the wire entity), linking a best-effort CLI-captured component scan (the `scan --local` upload of the scanned platform's installed capabilities) onto the agent run so the Agent Report projects that run's per-capability `scans` into the **Component Scores** tab; NULL on web / `--print-skill` paths (no local filesystem); `SET NULL` (not CASCADE) so deleting the component scan never deletes the behavioral run; lineage …→0021→0022→0023. See § Agent directory surface. `0022_drop_agent_verify_waitlist` (down-rev `0021_agent_kind_tally`) — drops the `agent_verify_waitlist` table + `idx_agent_verify_waitlist_created` and narrows `chk_rate_limits_bucket` back to the pre-0020 set (removes the `agent_verify_waitlist` bucket, purging any live counter rows first), reversing the demand-capture half of 0020 (the `agent_runs.vendor_reply*` right-of-reply columns stay) after the "Request independent verification" waitlist tile was removed from the Agent Report; lineage …→0020→0021→0022. `0021_agent_kind_tally` (down-rev `0020_agent_directory_surface`) — adds the nullable `agent_runs.kind_tally` JSONB column (the per-kind capability inventory `{skill,hook,mcp,plugin,rules}→count` backing the `/agents` dossier-card capability stack; DB-only `x-postgresql-extra-columns` on `agent-scan-report.schema.json`, projected onto the directory summary's `capability_tally` in `app/agent_scan/directory.py`, NOT on the generated wire entity / `openapi.json`; NULL for real scans until the submit/grade flow captures a component inventory → the directory coalesces NULL to an all-zero tally; lineage 0001→…→0019→0020→0021). See § Agent directory surface. `0020_agent_directory_surface` (down-rev `0019_agent_scan`) — the I-5.6 Phase C public web surfaces: adds the nullable `agent_runs.vendor_reply` (VARCHAR 1000) + `agent_runs.vendor_reply_at` (TIMESTAMPTZ) columns (the capability-token holder's ≤500-char public right-of-reply, persisted on the run + rendered read-only on the report — DB-only `x-postgresql-extra-columns`, projected onto the hand-written report DTO, NOT the generated wire entity; D-5.6-08), creates the internal hand-written `agent_verify_waitlist` store (the "Request independent verification" demand-capture tile — redacted IP + optional email, retained), and adds `agent_verify_waitlist` to `chk_rate_limits_bucket` (lineage 0001→0006→0008→0016→0019→0020). See § Agent scan + `privacy.md` § agent_verify_waitlist. `0019_agent_scan` (down-rev `0018_scan_install_spec`) — the I-5.5 Agent Scan subsystem: creates 2 native enum types (`agent_run_status` = created/fetched/submitted/graded/published/aborted; `agent_test_verdict` = vulnerable/not_observed/n_a/error) via the idempotent 0009 DO-$$ pattern, the 2 schema-backed tables (`agent_runs`, `agent_findings`) + the 3 internal stores (`agent_evidence`, `agent_run_token_spent`, `agent_scan_telemetry`), reuses the existing `tier`/`severity`/`visibility` enums, and adds `agent_scan_submit` to `chk_rate_limits_bucket` (lineage 0001→0006→0008→0016→0019). See § Agent scan below. `0018_scan_install_spec` (down-rev `0017_skill_compat_codex`) — adds the nullable `scans.install_spec` JSONB column (the per-capability install descriptor the `saferskills` CLI consumes to install/uninstall/update **every** capability kind across compatible agents; derived from the same already-public scanned bytes via `app/scan/discovery.py::build_install_spec`, persisted in `persistence.py::_apply_scan_result`, surfaced on the report DTO). JSONB → no `CREATE TYPE`/`KNOWN_ENUMS`; backfill deferred (existing scans NULL; the `rescan_rules` trigger repopulates from stored bytes). Stored-snapshot tier, NEVER a scan-trace field (`security.md`). `0017_skill_compat_codex` (down-rev `0016_cli_pow_spent`) — a **data-only** backfill that re-derives `catalog_items.agent_compatibility` for every `kind='skill'` row to the widened skill-capable set `["claude-code","codex","copilot","gemini","openclaw"]` (mirrors `app/services/agent_compat.py::agent_compatibility_for` after Codex/Copilot/Gemini gained a native `skills/` surface; no schema change). `0016_cli_pow_spent` (down-rev `0015_install_events`) — creates the internal `cli_pow_spent` single-use ledger (`challenge_sha256` PK, `expires_at`, `created_at`; `idx_cli_pow_spent_expires_at`) backing the stateless CLI Proof-of-Work scan-submit gate (I-05, D-05-30), and drops+recreates `chk_rate_limits_bucket` adding the `cli_scan_submit` bucket (VARCHAR+CHECK, not a native enum). `0015_install_events` (down-rev `0014_scan_pipeline_redesign`) — creates the dedicated `install_events` table + the native `agent` PG enum (the existing `kind` enum from `0009` is reused) backing the real opt-in `install_activity` aggregate (I-05, D-05-31); `idx_install_events_item_created (catalog_item_id, created_at DESC)`; `catalog_item_id` FK `ON DELETE CASCADE`; rows retained (redacted IP, no PII). `0014_scan_pipeline_redesign` (down-rev `0013_add_ingestion_runs`) — the collect-and-scan redesign: (a) `scan_source` enum += `rescan_rules`; (b) `catalog_items` collapses the two scan-depth recency columns (`last_deep_scan_at`/`last_lite_scan_at`, added by `0012`) into the unified queue-of-record set `last_scanned_at` + `scanned_rubric_version` + `scanned_engine_version` + `last_checked_at` (`last_scanned_at` is backfilled from the old columns BEFORE they drop — no data loss); (c) the NEW internal `repo_fetch_state` table (per-repo conditional-fetch validators). `0013_add_ingestion_runs` creates the `ingestion_runs` table (one row per cycle attempt) backing the eagle-eye health view (`GET /admin/sources` enrichment + `…/{source}/runs`); `status`/`trigger` CHECKs + `idx_ingestion_runs_source_started (source, started_at DESC)` + partial `idx_ingestion_runs_running WHERE status='running'`; swept after 90 days by `app/core/sweeps.py`. `0012_phase_c_scan_recency` added the nullable `catalog_items.last_deep_scan_at` + `last_lite_scan_at` recency columns (now collapsed by `0014`). Chain: `0019_agent_scan` ← `0018_scan_install_spec` ← `0017_skill_compat_codex` ← `0016_cli_pow_spent` ← `0015_install_events` ← `0014_scan_pipeline_redesign` ← `0013_add_ingestion_runs` ← `0012` (scan recency) ← `0011` (outbox tables) ← `0010` (full projection) ← `0009` (native enums) ← `0008`. Reversible downgrades throughout.
...
- **Stale-running reaper (orphan recovery).** A worker reload BETWEEN `record_run_started` and `record_run_finished` orphans a row in `status='running'` forever — the eagle-eye view then reads the source as permanently `stuck` (`health.is_stuck` over `min(started_at) FILTER (status='running')`) and inflates the `running` count. `app/ingestion/tasks.py::mark_stale_ingestion_runs(session, *, cutoff)` flips every `running` row older than `cutoff` to `failed`/`StaleRunReaped`; `recover_stale_ingestion_runs` is the session-owning wrapper (grace = `INGESTION_STALLED_SECONDS`, default 4h — the SAME worst-case-cycle-safe constant `ingestion_stalled_retrier` uses). It runs at **boot** (`app/main.py`, right after `recover_stale_scans`) **and every 15 min** (the `ingestion_run_reaper` periodic task, `queue='periodic'`). Mirrors `scan_runner.recover_stale_scans` for the ingest side; **distinct from `ingestion_stalled_retrier`**, which re-queues abandoned Procrastinate *jobs* (a `running` run-record is not a job and has nothing to retry).
...
exact solved challenge has been spent", to block replay.

| Piece | Shape | Role |
|---|---|---|
| `cli_pow_spent` | `challenge_sha256` VARCHAR(64) PK; `expires_at` TIMESTAMPTZ NOT NULL; `created_at` TIMESTAMPTZ DEFAULT now(). Index `idx_cli_pow_spent_expires_at`. | Single-use replay guard — a solved challenge is claimed via `INSERT ... ON CONFLICT DO NOTHING`; a replay inserts 0 rows → 403 (silent, no duplicate-key ERROR log). |

- **Internal storage only** — NOT part of the generated entity pipeline (no `schemas/*.schema.json`, no Pydantic/Zod/TS DTO, no wire model). Hand-written `app/models/cli_pow_spent.py`, registered in `app/models/__init__.py`. The endpoint DTO (`CliChallengeResponse`) is a hand-written wrapper.
- **Verify is `app/services/cli_pow.py`** — constant-time HMAC compare, expiry check, leading-zero-**bits** ≥ `cli_pow_difficulty` on `sha256(challenge‖solution)`, then the single-use `INSERT ... ON CONFLICT DO NOTHING` (rowcount 0 = replay → 403; the row commits with the downstream rate-limit upsert). The exact wire byte-layout is documented there + mirrored in `cli/src/core/pow.rs` (a cross-checked unit test on both sides locks the interop). No payload, no PII — only a hash + an expiry.
- **`chk_rate_limits_bucket` gains `cli_scan_submit`** (the PoW-path per-IP cap, distinct from the Turnstile `scan_submit` bucket). VARCHAR+CHECK (the `rate_limits` table is internal hand-written, not native-enum) — lineage 0001 → 0006 → 0008 → 0016.
- **Retention: swept by `sweep_cli_pow`** (§ Expiry sweep, lock `0x5AFE5C12`) once `expires_at` passes — the challenge is invalid after expiry anyway.
...
| Piece | Shape | Role |
|---|---|---|
| `agent_runs` | `id` PK; `status` (native `agent_run_status` enum — created/fetched/submitted/graded/published/aborted); `visibility` (reuses the `visibility` enum) + `share_token` (NULL UNIQUE) + `expires_at` (unlisted-only TTL, `UNLISTED_AGENT_RETENTION_DAYS`); aggregate `repo_tier` (reuses `tier`)/score; `rubric_version`/`engine_version`; timestamps | The agent scan. `GET /agent-scans/{run_id}` is the public projection (NEVER loads `agent_evidence`). |
...
- **2 native enum types are NEW in 0019** (`agent_run_status`, `agent_test_verdict`); `tier`/`severity`/`visibility` are reused. A new enum value = update the closed set + a migration.
- **`chk_rate_limits_bucket` gains `agent_scan_submit`** (the agent-scan-submit per-IP cap) — VARCHAR+CHECK, lineage 0001 → 0006 → 0008 → 0016 → 0019.
...
- **`agent_runs.vendor_reply` (VARCHAR 1000, nullable) + `agent_runs.vendor_reply_at`
  ...
  hand-written `AgentScanReportDetail` DTO + `app/agent_scan/report.py`. Stored
  vendor text, NEVER a finding/trace field (`security.md` no-raw-payload invariant
  is unaffected — this is a vendor-authored reply, not artifact payload).

> **Removed (migration 0022).** 0020 also created the internal `agent_verify_waitlist`
> store + the `agent_verify_waitlist` rate-limit bucket, backing the "Request
> independent verification" waitlist tile on the Agent Report. The tile was removed
> full-stack, so **0022 drops the table + index and narrows `chk_rate_limits_bucket`
> back to the pre-0020 set** (the `vendor_reply*` columns are untouched). No
> `agent_verify_waitlist` model, endpoint, or bucket remains.

**`agent_runs.kind_tally` (JSONB, nullable, migration 0021)** — the per-kind
capability inventory (`{skill,hook,mcp,plugin,rules}→count`) that backs the
`/agents` dossier-card capability stack (`CapabilityStack`). Added as
**`x-postgresql-extra-columns`** on `agent-scan-report.schema.json` (DB-only — the
SQLAlchemy generator adds the column; it is NOT on the generated wire entity /
...
**`agent_runs.component_scan_run_id` (UUID, nullable, FK → `scan_runs(id)` `ON
DELETE SET NULL`, migration 0023)** — links a best-effort, CLI-captured **component
scan** (the `scan --local` upload of the scanned platform's installed capabilities,
visibility-mirroring the agent run) onto the agent run. The Agent Report's
**Component Scores** tab is projected live from that scan_run's per-capability
`scans` (joined to their `catalog_items`) by `app/agent_scan/components.py::
load_component_scores` — each → an `AgentComponentScoreRow{kind,name,path,score,
tier,slug}`. For an unlisted agent run the row deep-links resolve to the unlisted
component scan_run's `/scans/r/<token>` report (its shadow catalog items 404 on the
public catalog); a public run links each row to its real `/items/<slug>`. DB-only
`x-postgresql-extra-columns` on `agent-scan-report.schema.json` (the generated
`AgentRun` model carries the `mapped_column`; NOT on the wire entity / `openapi.json`
— `component_report_url` is a hand-written report-DTO field). NULL on web /
`--print-skill` paths → the tab keeps its honest empty state. `SET NULL` (not
CASCADE): deleting the component scan must NOT delete the behavioral run, only drop
the dangling link. Captured `kind_tally` survives grading (both are set at bootstrap).

The public-only directory queries live in `app/agent_scan/directory.py`
(`list_public_runs` + `aggregate_stats`) — they read `agent_runs` directly (an Agent
Report is its own entity, never a `catalog_items` row) and hard-filter
`visibility='public' AND status IN ('graded','published') AND score IS NOT NULL`.
...
| Stale-running reaper grace / cadence change | "Ingestion runs" § Stale-running reaper + `app/ingestion/tasks.py` (`mark_stale_ingestion_runs`/`ingestion_run_reaper`) + `app/main.py` boot hook + `INGESTION_STALLED_SECONDS` |
...
| `scans.install_spec` shape / extraction change | "Migrations" head note + `schemas/scan-report.schema.json` (`installSpec`) + `app/scan/discovery.py::build_install_spec` + `app/scan/report_builder.py` + the CLI `InstallSpec` DTO (`cli/src/api/dto.rs`) + `security.md` (trace invariant) |
| Agent-scan table / enum / cascade / retention change | "Agent scan" + the model under `app/models/` (`agent_evidence.py` / `agent_run_token_spent.py` / `agent_scan_telemetry.py` or the generated `AgentRun`/`AgentFinding`) + migration 0019 + `privacy.md` (telemetry) + `security.md` (no-raw-payload trace) |
| New internal (hand-written) model added | "Models are codegen-driven" — extend the seventeen-store list + `app/models/__init__.py` docstring; confirm it has no wire DTO (else it must be schema-driven/generated) |
