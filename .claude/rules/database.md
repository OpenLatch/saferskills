...
- **Every migration is reversible** — `downgrade()` drops what `upgrade()` adds. Current head: `0014_scan_pipeline_redesign` (down-rev `0013_add_ingestion_runs`) — the collect-and-scan redesign: (a) `scan_source` enum += `rescan_rules`; (b) `catalog_items` collapses the two scan-depth recency columns (`last_deep_scan_at`/`last_lite_scan_at`, added by `0012`) into the unified queue-of-record set `last_scanned_at` + `scanned_rubric_version` + `scanned_engine_version` + `last_checked_at` (`last_scanned_at` is backfilled from the old columns BEFORE they drop — no data loss); (c) the NEW internal `repo_fetch_state` table (per-repo conditional-fetch validators). Its downgrade reverses to `0013_add_ingestion_runs` (rebuilds the depth columns + recreates `scan_source` without `rescan_rules`) and leaves `ingestion_runs` untouched. `0013_add_ingestion_runs` creates the `ingestion_runs` table (one row per cycle attempt) backing the eagle-eye health view (`GET /admin/sources` enrichment + `…/{source}/runs`); `status`/`trigger` CHECKs + `idx_ingestion_runs_source_started (source, started_at DESC)` + partial `idx_ingestion_runs_running WHERE status='running'`; swept after 90 days by `app/core/sweeps.py`. `0012_phase_c_scan_recency` added the nullable `catalog_items.last_deep_scan_at` + `last_lite_scan_at` recency columns (now collapsed by `0014`). Chain: `0014` ← `0013_add_ingestion_runs` ← `0012` (scan recency) ← `0011` (outbox tables) ← `0010` (full projection) ← `0009` (native enums) ← `0008`. Reversible downgrades throughout.
...
| New internal (hand-written) model added | "Models are codegen-driven" — extend the twelve-store list + `app/models/__init__.py` docstring; confirm it has no wire DTO (else it must be schema-driven/generated) |
