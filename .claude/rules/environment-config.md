...
| `DB_POOL_SIZE` | no | `5` | SQLAlchemy persistent pool size. The API + every ingestion task draw sessions from here. Lowered from 10 to keep idle-connection RAM modest on the `shared-cpu-1x:256MB` Postgres; scale up by config with prod RAM. |
...
| `SCAN_SUBMIT_DAILY_LIMIT` | no | `10` | Max scan submissions per IP per 24h, **shared by `POST /scans` and `POST /scans/upload`**. Loopback callers are exempt (trusted local seeding) — see `security.md` § Public-input handling #5. |
...
| `SAFERSKILLS_SITE_ORIGIN` | no | `https://saferskills.ai` | Canonical public origin for absolute `<loc>` URLs in the generated sitemap (`/api/v1/sitemap/*`) and the IndexNow URL set. Always the real `saferskills.ai` apex (clean crawler-facing URLs) — distinct from `PUBLIC_BASE_URL`'s dev/preview origin. Non-secret; set per-env in `fly.*.toml [env]` if it ever differs. See `security.md` § Public-input handling. |
| `SAFERSKILLS_INDEXNOW_KEY` | no | unset | IndexNow API key. Pings Bing/Yandex/DuckDuckGo/Seznam/Naver/Yep (**NOT Google**) on new public, real-data pages via a Procrastinate `ping_indexnow` job. Unset (dev/test/CI) → the submitter (`app/seo/indexnow.py`) is a no-op. The key file is served statically at `webapp/public/<KEY>.txt` (provisioned out-of-band). **Delivery:** a SECRET — `flyctl secrets set --stage` in `deploy.yml` (API apps). |
| `SAFERSKILLS_INDEXNOW_KEY_LOCATION` | no | unset | Absolute URL of the IndexNow key verification file (`https://saferskills.ai/<KEY>.txt`), sent as `keyLocation` in the IndexNow POST body. Unset → omitted (engines look for the key at the host root). Pairs with `SAFERSKILLS_INDEXNOW_KEY`. |
...
| `VENDOR_SESSION_SECRET` | yes (staging/prod) | dev-insecure default | HS256 signing key for the vendor right-of-reply session JWT (`ss_vendor_session` cookie). The API is the sole verifier — the webapp stores the JWT opaquely and forwards it as a Bearer token. 32+ random bytes in prod; rotate quarterly (rotation only invalidates in-flight 15-min sessions). A `model_validator` in `config.py` **hard-fails boot** when it is still the public dev default (or <32 bytes) and `ENV` is `staging`/`production` — the default ships in the open-source repo, so a deploy that kept it would mint forgeable verified-vendor sessions (mirrors `TURNSTILE_SECRET_KEY` / PoW). **Delivery:** a SECRET — `flyctl secrets set --stage` in `deploy.yml` (API **and** worker apps, since both load `Settings` and run the guard) from a **repo-level** GH Actions secret (same value both envs). |
...
| `INGESTION_WORKER_ENABLED` | no | `true` | Start the in-process Procrastinate ingestion worker (advisory lock `0x5AFE5C13`). **Deployed posture (the worker split, `ingestion.md` § Procrastinate worker):** the API `fly.*.toml [env]` set this `false` (the deployed worker runs as its own process — `services/worker/`, `python -m app.worker_main` — set `true` there). The default stays `true` so local dev + `docker compose up` run everything in one process. Also set `false` in test contexts that must not fan out external fetches. The API still opens the Procrastinate connector when this is `false` so its own defer paths work. |
| `INGESTION_WORKER_CONCURRENCY` | no | `4` | Procrastinate worker concurrency. INVARIANT: `INGESTION_WORKER_CONCURRENCY + SCAN_MAX_CONCURRENCY` must stay below `DB_POOL_SIZE + DB_MAX_OVERFLOW` (5 + 10 = 15), leaving API headroom; **asserted at startup** (`app/ingestion/worker.py::assert_worker_concurrency_budget` — the API refuses to boot otherwise). The single in-process worker drains both the ingest queues and the `scan` queue (concurrency = the sum); scan jobs are separately capped by `SCAN_MAX_CONCURRENCY`. See `tech-stack.md` § Procrastinate mandate. |
...
| `SCAN_RECONCILE_MAX_BACKLOG` | no | `500` | Back-pressure ceiling on the `scan`-queue backlog (count of `todo` `scan_capability_repo` jobs in `procrastinate_jobs`). When `auto_scan_reconcile` sees a backlog at/above this it SKIPS the tick — never piling more onto a worker already behind, the missing back-pressure that lets the shared-Postgres staging box saturate. Staging stages `200` in `services/worker/fly.staging.toml [env]`. See `ingestion.md` § Durable auto-scan pipeline. |
...
| `SCAN_MAX_INDEX_FILES` | no | `4000` | Per-repo ceiling on files admitted to the in-memory scan file index, on EVERY fetch path (tarball walk + Git Trees); extras are skipped gracefully (recorded on the report). Bounds a many-small-file monorepo's footprint. |
| `SCAN_MAX_INDEX_TOTAL_BYTES` | no | `26214400` | Per-repo total-bytes ceiling (25 MiB) on the in-memory scan file index, on EVERY fetch path. The 25 MiB tarball cap is COMPRESSED-stream only — this bounds what actually sits in RAM. Applied in sorted-path order via `fetch.IndexBudget`, so the kept fileset is fetch-path-independent. |
...
| `GITHUB_APP_ID` | no | unset | GitHub App `saferskills-ingest` numeric App ID. Required in production for the ingestion worker. |
| `GITHUB_APP_PRIVATE_KEY` | no | unset | GitHub App PEM private key (RS256, multi-line). Accepts a **raw PEM** OR a **base64-encoded PEM** — `config.py::_normalize_github_app_private_key` decodes the latter at load. In deployment the single-line `GITHUB_APP_PRIVATE_KEY_B64` secret is used instead (multi-line Fly secrets are awkward to set). **Without a working key the API + worker hit GitHub anonymously (60 req/h)** — ingestion enrichment is throttled (empty-tier items hidden from the catalog) and the auto-scan pipeline 429s. |
| `GITHUB_APP_PRIVATE_KEY_B64` | no | unset | Base64-encoded GitHub App PEM — the deployed shape of the key above (a single-line Fly secret). Decoded into `github_app_private_key` at load when the raw key is unset (`config.py::_normalize_github_app_private_key`). This is the secret actually set on the `saferskills-api*` / `saferskills-worker*` apps. |
...
| `INGESTION_GITHUB_CODE_SEARCH_ENABLED` | no | `false` | Enable the `github_topics` code-search discovery pass. Default off. |
| `SLACK_ALERTS_WEBHOOK_URL` | no | unset | Slack incoming-webhook URL for `#saferskills-alerts` (ingestion failure alerts). |
| `SLACK_INVITE_URL` | no | unset | Never-expire invite to the shared `openlatch-community` Slack workspace — the `GET /api/v1/community/slack/redirect` target (backs the webapp `/slack` pretty URL). The value is semantically **public** (anyone with it can join), but **delivered as a secret** so the join link stays out of git + is rotatable without a commit. Validated to an `https://*.slack.com` host at config load (`config.py::_validate_slack_invite_url`; blank → treated as unset, so an unconfigured deploy boots and the redirect 503s). **Delivery:** staged via `flyctl secrets set --stage` in `deploy.yml` (**API apps only** — the redirect + health probe run in the web tier, not the worker/webapp) from the **repo-level** GH Actions secret `SLACK_INVITE_URL` (same value both envs, like `SLACK_ALERTS_WEBHOOK_URL`); skipped when unset. Local dev still reads `.env` / `docker-compose.yml`. |
| `SLACK_INVITE_HEALTH_INTERVAL_SECONDS` | no | `21600` (6h) | Interval of the in-process Slack-invite health probe loop (`app/services/slack_invite_health.py`, advisory lock `0x5AFE5C14`). On a broken invite (HTTP ≥400 or a "no longer active" body) it alerts via `SLACK_ALERTS_WEBHOOK_URL` + Sentry. No-ops when `SLACK_INVITE_URL` is unset. |
| `SAFERSKILLS_ADMIN_KEY` | no | unset | Shared secret gating `GET/POST /api/v1/admin/*` via the `X-Admin-Key` header. Generated by `saferskills-admin auth gen-admin-key`. Unset + `ENV=development` → keyless local access (no `X-Admin-Key` needed; audits as `local-dev`) so the operator CLI works on a dev machine; unset + staging/production → every admin endpoint returns 403 (fails closed). **Delivery:** a SECRET — staged via `flyctl secrets set --stage` in `deploy.yml` (API apps) from an **environment-scoped** GH Actions secret (distinct value per env), never a committed `[env]`. Replaced by SSO when auth lands. |
| `SAFERSKILLS_AGENT_MASTER_KEY` | yes (staging/prod) | unset | Master key the Agent Scan subsystem derives per-run canaries from (HKDF Extract+Expand, `app/agent_scan/canary.py`). Unset (dev) → packs serve with a dev canary key. A `model_validator` in `config.py` **hard-fails boot** when unset and `ENV` is `staging`/`production` (mirrors `TURNSTILE_SECRET_KEY` / PoW). **Delivery:** a SECRET — `flyctl secrets set --stage` in `deploy.yml` (API apps) from a **repo-level** GH Actions secret (identical staging+prod, so the one baked CLI pubkey verifies everywhere). See `security.md` § Scan-trace transparency. |
| `SAFERSKILLS_PACK_SIGNING_KEY` | yes (staging/prod) | unset | Ed25519 private key used to sign the served Agent Scan pack bytes (`app/agent_scan/signing.py`, key_id `saferskills-agent-pack-2026`). Unset (dev) → packs serve **unsigned**. Same `model_validator` boot-guard as `SAFERSKILLS_AGENT_MASTER_KEY`. **Delivery:** a SECRET — `flyctl secrets set --stage` in `deploy.yml` (API apps) from a **repo-level** GH Actions secret (identical staging+prod). |
| `SAFERSKILLS_PACK_PUBKEY` | no | unset (dev) | **CLI build-time bake** (the public counterpart of `SAFERSKILLS_PACK_SIGNING_KEY`), NOT a backend/runtime var. A comma-separated `<key_id>=<base64-std-pubkey>` map baked into the `saferskills` binary by `cli/build.rs` (like `SAFERSKILLS_POSTHOG_KEY`) so `scan agent` can `verify_strict` the served pack pre-flight. **Empty (dev/fork build) → verification is skipped with a warning** (`manual-bootstrap` posture); a release build with a baked key **fails closed** (a missing/unknown signature aborts). **Delivery:** a `--build-arg`/env at CLI build in `publish-npm.yml` from the repo-level GH Actions secret `PACK_PUBKEY`. See `security.md` § Scan-trace transparency. |
...
| `UNLISTED_AGENT_RETENTION_DAYS` | no | `90` | TTL for unlisted Agent Scan runs — sets `agent_runs.expires_at`; swept like the unlisted scan runs. |
...
| `AGENT_CORPUS_GATE_N` | no | `500` | Minimum public agent-scan corpus before the `/agents` directory publishes its headline aggregate stat (% carrying a critical finding); below it the stat is gated ("collecting") to avoid misleading-at-small-N. Read by `GET /agent-scans/aggregate-stats` (`app/agent_scan/directory.py`). Founder-overridable without a code change. |
...
## Fly secret-staging rule (when deploy ships)
