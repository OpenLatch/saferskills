...
| `INGESTION_WORKER_CONCURRENCY` | no | `4` | Procrastinate worker concurrency. INVARIANT: must stay below `DB_POOL_SIZE + DB_MAX_OVERFLOW` (5 + 10 = 15), leaving API headroom; **asserted at startup** (`app/ingestion/worker.py::assert_worker_concurrency_budget` — the API refuses to boot otherwise). See `tech-stack.md` § Procrastinate mandate + crash-resilience addendum §1.5. |
...
| `SAFERSKILLS_ADMIN_KEY` | no | unset | Shared secret gating `GET/POST /api/v1/admin/*` via the `X-Admin-Key` header (Phase C, D-04-28). Generated by `saferskills-admin auth gen-admin-key`. Unset (dev/test) → every admin endpoint returns 403 (fails closed). **Delivery:** a SECRET — staged via `flyctl secrets set --stage`, never a committed `[env]`. Replaced by SSO when auth lands (Track E). |
