...
| **CLI crate stack** | clap 4.6 + clap_complete + clap_mangen; inquire; indicatif; ratatui 0.29 + crossterm 0.28 (`event-stream`) + nucleo 0.5 + futures-util (the `search` TUI); reqwest 0.12 **rustls-only**; tokio (`rt`,`rt-multi-thread`,`macros`,`sync`,`time`); toml/toml_edit + jsonc-parser; zip 8; strsim; anyhow + thiserror + miette; dirs; serde/serde_json | Mirrors `openlatch-client` where it chose; gaps filled per I-05 design §7. The `search` TUI adds ratatui (draws to **stderr**; stdout stays machine-clean) + crossterm async `EventStream` + nucleo (synchronous fuzzy re-rank of the loaded pool) + futures-util — all TLS-free, so `cli-rustls` stays green; release binary measured **~5.1 MB** (well under the 15 MB `cli-build` gate; the `nucleo-matcher`/trimmed-feature fallback was not needed). `[profile.release] opt-level="z", lto, codegen-units=1, strip, panic="abort"`. rustls-only is CI-enforced (`cli-rustls` lane greps for `openssl-sys`/`native-tls`). |
