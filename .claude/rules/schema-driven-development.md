...
| 7 | **methodology** | `rubric/<CATEGORY>/<NAME>-NN.md` (YAML frontmatter) | `webapp/src/generated/methodology/index.mdx` + `webapp/src/generated/methodology/rule-count.ts` + `webapp/src/generated/rules/content.ts` (the `RULE_CONTENT` explainable-finding map) **+ `services/api/app/generated/rule_content.json`** (snake_case backend mirror served by `GET /api/v1/rubric/content` for the install CLI's offline finding prose — D-05-32; emitted from the SAME rule walk so the two never drift) | `scripts/generate-methodology.cjs` (validates frontmatter against `schemas/rubric-rule.schema.json`; stamps rubricSha via `git log -n 1 -- rubric/`) |
