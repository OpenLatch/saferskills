...
│   │                 # EmailCaptureForm (retained — reused by the magic-link surface),
│   │                 # SegmentedTabs, Toggle, Select (DS listbox — replaces native <select>),
...
│   │                 #   StalePackBanner (Agent Report score-hero + lifecycle atoms),
│   │                 # RefChip, ProvenanceChips (deep-linked ref chips + badge provenance)
│   │                 # RuntimeMonogram, CapabilityStack (/agents directory dossier atoms)
│   ├── molecules/    # NavBar, CtaBand, AgentMarquee, WhyRow, InstallTabs, ActionCard,
│   │                 # RecentScanCard, TrendScanCard
...
│   │                 # VendorResponseCard
│   │                 # DropZone (animated upload state machine)
...
│   │                 # ProofOfTestsTable (Report-tab proof-of-tests; reuses .chk-* grammar),
│   │                 #   RightOfReplyForm (Agent Report lifecycle),
│   │                 #   AgentCleanVerdict (Findings-tab clean-pass empty state — `.ar-empty` panel)
│   │                 # OwaspFindingGroup, ScoreMathTable, RemediationTerminal, RedactedTranscript,
│   │                 #   ComponentScoresTable (Findings/Component tabs)
│   │                 # CorpusRiskMeter, RiskThermometer, AgentFilterBar, MultiSelect,
│   │                 #   DossierCard (/agents directory)
│   │                 # PromptCodeCard (the code-editor-chrome bootstrap-prompt block,
...
The v3 mockups (`SaferSkills-Scan-Results-{Private,File}-v3`) **consolidate** the flagged-finding presentation into one shared molecule, **superseding** the earlier "FindingRow vs CheckGroupList — do NOT consolidate" rule (and retiring `FindingRow` + its container `SubScoreAccordion`, both now removed).
...
### `.mf-*` file-tab strip is a page-specific tablist
...
- **DS-component CSS lives in `ui/styles/components.css`** (the one-way CSS-ownership rule): every class rendered by a `ui/` agent component — `.ar-verb`/`.vb-sep` (`AgentVerb`), `.trust-pill`/`.ti`/`.tip` (`TrustTierPill`), `.cap-reason`/`.cr-ic` (`CapCallout`), `.ar-tests` + the `.chk-gobtn`/`.chk-row.fail` **additions scoped under `.ar-tests`** so the shared scan-report `CheckGroupList` is untouched (`ProofOfTestsTable` — per the locked mockup it is the `score-checks-head` + plain `.chk-group`s, no celebratory header), `.evidence-public-note` (`EvidenceWithheldNote`), `.ar-stale-banner` (`StalePackBanner`), `.ror-*` (`RightOfReplyForm`). **The OWASP-grouped findings view adds**: `.owasp-group*`/`.og-*` (`OwaspFindingGroup`), `.ref-chip`/`.fc-refs` (`RefChip` — rendered on the OWASP group head; per the locked mockup the finding card body carries no refs row), `.score-math`/`.sm-*` (`ScoreMathTable`), `.ar-remediation`/`.ar-rem-*`/`.ar-fix-snippet`/`.ar-term`/`.at-*` (`RemediationTerminal`), `.transcript`/`.ts-*` + `.transcript .ex-line.canary` (`RedactedTranscript`, reusing the dark `.ex` line-window), `.ar-components-tab` (`ComponentScoresTable` — the rows reuse the scan report's `.cap-list`/`.cap-row` grammar from `page-scan-report.css`, exactly as the mockup does), `.prov-chip`/`.ar-prov-chips` (`ProvenanceChips`). **The two empty states share one panel system** — `.ar-empty` (bordered report section on a faint dot-grid field) with `--pass` (Findings clean-pass verdict — hex-cap seal `.cv-seal` + `.cv-eyebrow`/`.cv-title`/`.cv-lede` + the run's real `.cv-ledger`/`.cv-stat`/`.cv-num` + `.cv-prov` reproducibility line; `AgentCleanVerdict`) and `--na` (Component-Scores not-in-this-scan explainer — `.csx-kinds`/`.csx-chip` ghost preview + `.csx-link`; `ComponentScoresTable`'s empty branch, superseding the old bare `.cs-empty` line), staggered `ar-empty-rise` entrance, reduced-motion guarded. Ported from the locked mockup with `body.va.dark → html.dark`, tokens only, reduced-motion guards on every transition; the `.ex`/`.ar-term`/`.transcript` terminal surfaces use the sanctioned components.css terminal palette (intentional `#fff`/diff hex).
- **`webapp/src/styles/page-agent-report.css`** owns ONLY page-composition glue (`.ar-scoreline` hero row, `.ar-panel`/`.ar-panel-lead` tab panels, `.ar-tab-placeholder`, `.ar-lifecycle`, the Phase-B `.ar-findings-head`/`.ev-cap`/`.ar-export-fixes` Findings-tab header + the `.ar-vendor-reply`/`.avr-*` read-only reply figure; the badge band reuses the shared full-bleed `.badge-band`/`.badge-wrap` from `page-scan-report.css` — the mockup layout — wrapping `EmbedBadgeBox` + `ProvenanceChips`, with the unlisted badge-locked variant mirroring `ShareResultBand`) — it is a **`check-css` SHELL file** (registered in `scripts/check-css.cjs::SHELL_FILES`; token-only, no raw hex). The `--brand-primary-light` semantic alias the report references is defined once in `tokens.css` (`var(--ol-brand-primary-light)`).
- The report body is the React island `webapp/src/components/agent/AgentReport.tsx` (page-specific composition), mounted `client:load` (SSR'd + hydrated); the Findings tab + badge band are sub-compositions `webapp/src/components/agent/AgentFindings.tsx` + `AgentBadgeBand.tsx`, with the pure DTO→view mapping (ref deep-links, OWASP grouping, score-math ledger) in `webapp/src/lib/agent/findings-view.ts`. Telemetry (`agent_report_*`) + the manage/reply fetches + the client-side Markdown export (`webapp/src/lib/agent-report-markdown.ts`, full report + remediation checklist) live in the island; the `ui/` molecules stay presentational (callback props). The Findings card body reuses the DS `.find-card` chrome but composes the agent-specific body (refs → score-math → route-driven evidence → terminal remediation) — `FindingDetail` itself is not reused (its repo-scan body shape — occurrences/sha/github/reveal-invisibles — does not fit the behavioral finding). The **agent `EmbedBadgeBox` variant** (`kind="agent"`) emits `/badge/agent/{id}/{score}.svg` + `/agents/{id}`.

### Agent directory vocabulary

The `/agents` directory is a page-specific composition (only `webapp/src/pages/agents/index.astro` + its `AgentDirectoryApp` island render it). **All directory classes live in `webapp/src/styles/page-agent-directory.css`** — `.obs-*` (`CorpusRiskMeter` + the loading/empty/sentinel states), `.therm*` / `.distrib*` (`RiskThermometer`), `.filterbar` / `.fb-*` + `.ms-*` (`AgentFilterBar` / `MultiSelect`), `.card-grid` / `.dossier` / `.d-*` / `.caps-stack` / `.cap2` (`DossierCard` / `CapabilityStack`), `.rt-mono` (`RuntimeMonogram`), `.sk-card` / `.ee-*` (states). This mirrors the documented `.cap-filter` / `.mf-*` precedent — page-specific composition CSS stays in the page file even when rendered by components (here the components are directory-only). `page-agent-directory.css` is a **`check-css` SHELL file** (token-only colors/fonts, literal px spacing; registered in `scripts/check-css.cjs::SHELL_FILES`). The corpus aggregate is gated until `AGENT_CORPUS_GATE_N`; the grid is **latest-first** default with an opt-in sort + **infinite scroll** (IntersectionObserver sentinel → `All N shown` → an `END OF INDEX` `RidgeStars`). Reduced-motion guards the pulse/shimmer/thermometer-fill.
...
- **`circuit`** (`/agents/scan`) — a relay/circuit-trace seam: right-angle traces with square junction pads over a faint plus-grid field, plus registration `+` marks.
...
Every page route is pixel-diffed against the matching local mockup-shot reference at 1440×900, 1920×1080, 375×812. Tool: `tools/visual-diff/`. Thresholds: <0.5% per component, <1% per page. No iteration cap. **Local-only — not a CI lane** (mockup baselines are gitignored under `.local/`). PR descriptions record final diff ratios.
...
- Email risk-alerts get a single closing line about OpenLatch's runtime enforcement — the lone exception.
