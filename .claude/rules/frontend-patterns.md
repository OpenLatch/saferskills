...
├── scan/index.astro       → /scan               (umbrella scan page — SSR; `?mode=agent` deep-link)
...
├── agents/scan.astro      → /agents/scan        (platform-picker activation page — prerendered; the static segment beats /agents/[id])
├── agents/[id].astro      → /agents/:id         (public Agent Report; 404s unlisted runs)
├── agents/r/[token].astro → /agents/r/:token    (unlisted Agent Report — SSR, noindex, no-store)
...
├── research/state-of-ai-agent-skill-security.astro → /research/state-of-ai-agent-skill-security  (evergreen State-of report; prerendered; Dataset JSON-LD; SEO-O4)
├── 500.astro              → (branded SSR error page; prerendered, fetches NOTHING, Base noindex; L-7)
└── appeal.astro           → /appeal       (planned)
```

The `/research/state-of-ai-agent-skill-security` page (SEO-O4) is the hand-authored, evergreen "State of AI Agent Skill Security" census — mirrors the `/about` template (PageHead/PageRidge/`.page-section` bands + CtaBand + Footer), carries an answer-first 40–80-word lead in visible HTML, and emits one `Dataset` JSON-LD (Apache-2.0 license, via `datasetJsonLd`). Its copy + locked headline live in `webapp/src/lib/research/state-of.ts` so `research.test.ts` can pin them without rendering Astro; the headline X% ships as a `__PLACEHOLDER__` the founder fills before launch (D-07-06). It is listed in the static sitemap shard (`STATIC_ROUTES`).

`500.astro` (L-7) is the branded error page Astro's Node adapter renders on any uncaught SSR error (the middleware re-throws after `Sentry.captureException`). **Critical invariant — it fetches NOTHING** (a 500 happens exactly when the backend may be down, so unlike `404.astro` it never calls `getHomepageData()`): hardcoded chrome + copy, `prerender = true`, `<Base noindex>`, and a "Check status" link to `status.saferskills.ai`. The 502 `upstream_unreachable` JSON on the `/api/*` proxy path is a separate surface (proxy-down vs page-render-error).

The `/scan` page is one umbrella surface with a `[01 Capability | 02 Agent]` `SegmentedTabs` mode control owned by ONE island (`components/scan/ScanModeShell.tsx`, `client:load`): the Capability pane is the existing `ScanConsole` (v3 single-pane restyle — DropZone + "or paste a URL" divider + URL input; submit/validation/SSE logic untouched), the Agent pane is the shared activation island `components/scan/AgentScanActivation.tsx` (platform picker → Turnstile-gated mint via `useAgentScanMint` → the substituted bootstrap prompt). **`?mode=agent` is SSR-respected**: the page frontmatter reads `Astro.url.searchParams` and passes `initialMode` into the island AND the static methodology aside (`ScanMethodologyPreview.astro`, dual `.method-body[data-method]` bodies — the shell island toggles `hidden`, zero extra hydration), so a deep-link first-paints the Agent pane with no capability flash. Tab switches sync the URL with `history.replaceState` (no navigation). `/agents/scan` renders the same activation island (`surface='picker'`, `client:load`) on a prerendered shell; every marketing agent CTA targets `/scan?mode=agent`, so a `/agents/scan` rollback strands nothing.

The Agent Report routes (`agents/[id].astro` + `agents/r/[token].astro`) **mirror the `/scans` pair** — `prerender=false`; the unlisted route sets the same three anti-leakage headers + `Base noindex`, generic-404s a bad/expired token, and 307→`/agents/{id}` on a promoted run. The web pages are `/agents/*`; the **API** stays `/api/v1/agent-scans/*` (the backend `report.py` builds `report_url`/`share_url` at `/agents/*` so share links resolve). Both routes render the shared body island `webapp/src/components/agent/AgentReport.tsx` (`client:load`, SSR'd then hydrated): score hero (`ScoreNumber`-style `.sr-big`/dots/`CapCallout` — no band pill) + a `SegmentedTabs variant="underline"` shell (Report = `ProofOfTestsTable`; Findings/Component = placeholders, rendered later) + lifecycle chrome (manage bar + `RightOfReplyForm` on the unlisted token route only). The **evidence split is route-driven**: the public fetch helper guarantees + defensively strips `evidence_excerpt`; only the token route hydrates the transcript (rendered later). A pre-grade run renders `AgentScanPollBoard` (polls the page's own tokenless GET — NOT the token-gated `/{id}/status`). Dev builds against `fixtures/agent-scan-report.sample.json` via a `?fixture=` query (server-side, `import.meta.env.DEV`-gated). See `.claude/rules/security.md` § Capability-URL anti-leakage.

The unlisted capability-URL page (`scans/r/[token].astro`) sets all three anti-leakage headers at the **page** level (`Referrer-Policy: no-referrer`, `Cache-Control: private, no-store`, `X-Robots-Tag: noindex, nofollow`) + `Base noindex` (which also suppresses the token-bearing `canonical`/`og:url`). It reuses the same report body as `/scans/:id` via the shared `components/scan/ScanRunReport.astro`, which branches three ways: **single-cap upload** → rich score/source layout (mockups 3/4); **multi-file upload** → the same rich body per file behind a `FileTabStrip` (one tab per fanned-out capability); **repo scan** → the multi-capability cap-list table (`ScanReportView`, mockups 5/6). All uploads (1 or N) render via `components/scan/UploadReport.tsx`; the `isUpload` branch decides upload-vs-repo (not `isRich`, which now only gates the page-head). Unlisted runs add the private banner + `UnlistedManageBar` + `ExpiryCountdown`. The "Share this result" badge band is the shared `ShareResultBand.tsx` (React, reused by the repo path **and** per-file in `UploadReport`). See `.claude/rules/security.md` § Capability-URL anti-leakage.

### Structured data + social cards (SEO-T4 / T5)

- **JSON-LD** is built by pure functions in `webapp/src/lib/jsonld.ts` (typed with `schema-dts` so a malformed object fails `tsc`) and rendered by `webapp/src/components/seo/JsonLd.astro`. The renderer calls `serializeJsonLd` (the one place the `@context` is injected + the payload is XSS-escaped (`<` → `<`, and the U+2028/U+2029 separators → `\u2028`/`\u2029`) because a scanned repo's name/slug is attacker-controlled and `set:html` is used). Pass it one `schema-dts` Thing or an array (an array becomes one `@graph` block). `Organization` is site-wide in `Base.astro`; per-page schemas (`scannerAppJsonLd` on `/`, `capabilityAppJsonLd` + `breadcrumbJsonLd` on `/items/[slug]`) are emitted by the route. **Two hard rules**: (1) **NO fabricated `AggregateRating`/`Review`** — the 0-100 audit score lives in visible HTML, never as a schema rating (D-07-05; `jsonld.test.ts` guards it). (2) **Only emit `breadcrumbJsonLd` on a page that renders a VISIBLE `<Breadcrumb>`** — build it from the SAME crumb array (adapt `{label,href}` → `{name,path}`); fabricating a trail with no on-page counterpart is dishonest structured data. `/items/[slug]` qualifies (visible breadcrumb); `/scans/[id]` + `/agents/[id]` do **not** (they use `PageHead`/`page-head`, no breadcrumb) so they emit none. Structured data rides only on indexable pages (gate on the same predicate as `noindex`).
- **OG cards**: the dynamic `/og/{item,scan,agent}/[id].png` endpoints (`prerender=false`, `satori`+`@resvg` via `@/lib/og::renderOgCard`) mint the per-entity score card; the page wires `<Base ogImage={...}>` to it (helpers in `webapp/src/lib/seo/og-image.ts` keep the "completed → card, otherwise → static `/og-image.png` fallback" rule pure + tested). Every OG endpoint **404s an unlisted run** (anti-leakage — never mint a public image for a token-only run) and a not-yet-graded run (no score to card).
...
| Server data (when auth lands) | TanStack Query |
| Form state | `react-hook-form` + Zod resolver |

**There is no TanStack Query yet.** API reads happen via `fetch()` against `env.PUBLIC_API_URL`, with response shapes typed by the generated DTOs at `webapp/src/generated/openapi/types.gen.ts`. The Query layer arrives with auth.
...
Long-running scan progress is streamed from the backend via Server-Sent Events on `GET /api/v1/scans/<id>/events`. The frontend hook `webapp/src/lib/hooks/useScanProgress.ts` is the canonical pattern:
...
| TanStack Query arrives (auth lands) | "State management" + "Data fetching pattern" |
