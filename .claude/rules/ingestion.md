...
- **Memory budget (fits the 512 MB worker box).** `worker RSS ≈ ~200 MB baseline + SCAN_MAX_CONCURRENCY × (~40 MB per scan job — the bounded file index `SCAN_MAX_INDEX_TOTAL_BYTES` + decode/parse transients + the 4 MiB blob-insert chunk) + INGESTION_WORKER_CONCURRENCY × (~25-item batch incl. metadata_files)`. Staging runs `SCAN_MAX_CONCURRENCY=2` + `INGESTION_WORKER_CONCURRENCY=2` (≈ 320 MB) with `DB_POOL_SIZE=3`/`DB_MAX_OVERFLOW=4` to spare the 256 MB Postgres RAM. Per-job memory is bounded by: the in-memory file index cap on both fetch paths (§ Durable auto-scan pipeline), decode-/parse-once rule evaluation, the chunked `artifact_blobs` insert, and the per-batch `expunge_all` + `metadata_files` release (`registry_adapter._write_batch`). `app/core/memory.py::rss_mb` logs `memory.rss_mb` at each scan-job + cycle end for headroom tracking.
