...
paths:
  ...
  - contributor-docs/sources.md
---

# Ingestion

> **Paths**: `services/api/app/ingestion/**`, `schemas/ingestion-*.schema.json`, `schemas/merge-candidate.schema.json`, `tools/saferskills-admin/**`, `contributor-docs/sources.md`

A Postgres-backed ingestion pipeline. This rule loads only when ingestion files are edited — see `documentation-sync.md` § Rule files for the full rule inventory.
...
| `ScrapingAdapter` | Aggregator feed/sitemap/HTML scraping (implemented; see § Scrape fetch policy) |
...
- **Two deploy postures — same code, one process or two.** Locally + in `docker compose up` the worker runs **in-process** in the `app/main.py` FastAPI lifespan (`INGESTION_WORKER_ENABLED=true`, the default), alongside the expiry sweep + startup hooks. **In deployment** the worker runs as its **own Fly app** (`services/worker/` — a config-only deploy unit running the SAME api image with `python -m app.worker_main`); the API `fly.*.toml [env]` set `INGESTION_WORKER_ENABLED=false`, so the API tier is a lean web process and a worker OOM can't take it (or `/health`) down. The in-process design on one 512 MB box is what produced the staging OOM-loop. `app/worker_main.py` runs the worker-relevant subset of the lifespan (observability → migrations → pool → boot reapers → connector + schema → **orphaned-`doing`-job recovery** (`recover_orphaned_jobs_at_boot` — re-queues every `doing` job the dead prior worker left holding a `queueing_lock`) → supervisor → bounded teardown) and exits non-zero on an unmigrated DB so Fly's `[restart]` retries (no HTTP = no degraded-mode serving). **The API still opens the Procrastinate connector + applies the schema even with the worker disabled** (`app/main.py`) — its own defer paths (webhook dispatch, admin force-cycle, popularity recompute) call `defer_async` and would 500 on a closed connector. The expiry **sweep** (lock `0x5AFE5C12`) stays web-only. Both processes run migrations on boot (idempotent under lock `0x5AFE5C11`). See `services/worker/README.md`.
- **Memory budget (fits the 512 MB worker box).** `worker RSS ≈ ~200 MB baseline + SCAN_MAX_CONCURRENCY × (~40 MB per scan job — the bounded file index `SCAN_MAX_INDEX_TOTAL_BYTES` + decode/parse transients + the 4 MiB blob-insert chunk) + INGESTION_WORKER_CONCURRENCY × (~25-item batch incl. metadata_files)`. Staging runs `SCAN_MAX_CONCURRENCY=1` + `INGESTION_WORKER_CONCURRENCY=1` (≈ 265 MB) with `DB_POOL_SIZE=3`/`DB_MAX_OVERFLOW=4` to spare the 256 MB Postgres RAM (single-concurrency on both queues minimises the steady-state DB-connection draw that was saturating the shared PG and flaking the e2e-staging gate; `services/worker/fly.staging.toml [env]`). Per-job memory is bounded by: the in-memory file index cap on both fetch paths (§ Durable auto-scan pipeline), decode-/parse-once rule evaluation, the chunked `artifact_blobs` insert, and the per-batch `expunge_all` + `metadata_files` release (`registry_adapter._write_batch`). `app/core/memory.py::rss_mb` logs `memory.rss_mb` at each scan-job + cycle end for headroom tracking.
- **Supervisor restarts the worker on ANY unexpected stop; only cancellation exits** (`worker.py::ingestion_worker_supervisor`). Procrastinate self-stops the worker — `run_worker_async` returns **normally**, it does not raise — when a side task (the DB heartbeat) fails on a transient connection drop (a Postgres machine restart / Fly-proxy connection recycle). Because the worker runs with `install_signal_handlers=False` + `wait=True`, the ONLY intended exit is the lifespan cancelling the task (`CancelledError`); a plain return therefore means an unexpected self-stop and is restarted with backoff. Treating it as a graceful shutdown (the original `return`) let a routine PG blip permanently kill ingestion **and** the bulk-scan queue — and since the reaper / scheduler / `alert_evaluator` are themselves Procrastinate tasks, every safety net died with the worker, invisibly to Fly's HTTP-only health check. Pinned by `tests/test_ingestion/test_worker_shutdown.py::test_supervisor_restarts_on_self_stop`.
...
- The **interactive** SCAN path (`app/queue/scan_runner.py::scan_run_repo`, called from `POST /scans`) is **NOT Procrastinate** — it keeps its on-demand `asyncio.create_task` pattern. Only the **bulk** auto-scan (`tasks_scan.py`) is a Procrastinate job (a deliberate exception — see § Durable auto-scan pipeline + `tech-stack.md`). Do not migrate the interactive path.
...
- **Hishel 1.x RFC-9111 cache** backed by `AsyncSqliteStorage` at `HISHEL_DB_PATH` (Fly volume mount). LRU cap `HISHEL_MAX_SIZE_BYTES` (default 500 MiB). TTLs: `HISHEL_GITHUB_TTL_SECONDS` (default 24h) for `api.github.com` / `raw.githubusercontent.com`; `HISHEL_AGGREGATOR_TTL_SECONDS` (default 1h) for scraped hosts. **304 revalidations are free vs the GitHub rate limit.**
  - **One shared storage instance per cache-file path** (`http_client._SHARED_STORAGE`, keyed by `HISHEL_DB_PATH`), NOT one per `build()`. A fresh `AsyncSqliteStorage` per cycle opened a new SQLite connection each time, so concurrent ingest/scan fetches (each with their own per-instance write-lock) collided on the one cache file → `sqlite3.OperationalError: database is locked` flooding the worker (eviction `_batch_cleanup` never completed). One instance = one connection + one write-lock, so Hishel's own write serialisation actually applies. **Per-adapter TTL therefore travels as a per-request `hishel_ttl` httpx extension** (`_hishel_ttl_hook`, persisted per cache entry — `hishel_ttl` is exempt from Hishel's `hishel_*`-metadata strip), so sharing the store keeps the github-24h vs aggregator-1h retention without a junk wire header. `default_ttl` on the shared instance is only the (github) fallback.
...
## Scrape fetch policy (lean — no headless browser)
...
- **Discovery precedence: feed → sitemap → HTML.** `_fetch_feed` (JSON) and
...
- **No Playwright tier.** A Cloudflare interstitial (`is_cloudflare_challenge`:
  ...
  source is documented in `contributor-docs/sources.md`, not force-cracked.
- **Shared GitHub enrich.** Scrape adapters call `framework/github_enrich.enrich_repo_facts`
  ...
  feed without GitHub coordinates is a no-op (the item stays low-tier in the
...
- **Feed adapters:** `smithery` (feed `registry.smithery.ai/servers`,
...
- **HTML scrapers — one shared `SitemapHtmlAdapter`** (`framework/sitemap_scraper.py`).
...
> **Deps:** `curl_cffi` (browser-impersonating tier-1 fetch), `trafilatura`
> (`extract_main_content`), `beautifulsoup4` + `lxml` (DOM parsing), `defusedxml`
> (XXE-safe sitemap parse). **No Playwright / Chromium.** lxml is pinned `>=6.0.1` (the
> first release with cp314 wheels; the earlier `~=5.3` predates Python 3.14).

> **No seed migration needed for the 9 aggregator sources.** Migration `0011` already
> seeded one `crawler_cursors` row per source for all 14 (including the aggregators) and
> its CHECK constraints already include every aggregator value — so enabling an aggregator
...
## Agent-compatibility classifier
...
1. Founder runs the admin CLI: `uv run saferskills-admin sources pause <source> --reason "<reason>" --contact <email>`.
...
4. `contributor-docs/sources.md` is updated to reflect the change.
...
- **Alerts (`alert_evaluator`, every 15 min)** — per-source health from the `ingestion_events` outbox + `crawler_cursors.last_successful_cycle_at`, data-driven over the 14 YAML sources. **Warn** (failure_rate > 5% / 1h) → Sentry breadcrumb + PostHog `ingestion_cycle_failed`. **Page** (>25% / 1h OR >10% / 24h OR no successful cycle in 2× the YAML `cadence_cron`) → Slack `SLACK_ALERTS_WEBHOOK_URL`. A failure is any `http_status` not in {200, 304, 0}. The rug-pull/hash-change alert is not yet implemented.
  - **Platform label.** The page message is prefixed with `settings.env` (`[staging]` / `[production]`) since both envs post to the same channel — the `slack_invite_health` broken-invite page shares the prefix. Only the page POST changes; `alerts_page` still counts conditions met (dashboard semantics unchanged).
  - **Per-source cooldown (durable).** `alert_evaluator` is level-triggered + stateless, so a sustained condition would re-post every 15-min tick (the `#saferskills-alerts` npm-spam incident). A stable **signature** (`"|".join` of the active page reasons `silent`/`fr_1h`/`fr_24h`) plus the last-page timestamp live on the per-source `crawler_cursors.{alert_signature,last_alerted_at}` row (migration 0026 — durable so a worker restart doesn't reset the throttle and resume the spam). The Slack POST is **suppressed** when `INGESTION_ALERT_COOLDOWN_S > 0` and the same signature paged within the window; it re-pages once the window elapses or the signature changes. **Recovery** (no page condition) clears both columns to re-arm — there is **no** "✅ resolved" note (by decision). The cooldown only gates the POST; a `crawler_cursors` write commits once per tick iff any cooldown row changed. Residual root cause (documented, not fixed here): a single transient blip enters the `IngestionRetry` 1m→5m→30m→6h backoff whose `queueing_lock` can keep an hourly source quiet up to 6h → `silent_too_long` — the cooldown caps the *alerting* fallout from ~24 pages to ~1–2 per incident.

## Durable auto-scan pipeline

**Every indexed public-github capability is automatically + durably scanned.** Indexing leads to scanning — the catalog is never a wall of unscored rows. Lives in `app/ingestion/tasks_scan.py`; all three pieces run in the in-process Procrastinate worker. This is a **deliberate exception** to the on-demand scan pattern: bulk scan is queue-shaped (durable Procrastinate jobs); the interactive `POST /scans` path stays `asyncio.create_task` (latency-sensitive, SSE). See `tech-stack.md`.

| Piece | What it does |
|---|---|
| `scan_capability_repo` (queue `scan`, `retry=3`, per-defer `queueing_lock`/`lock` = `scan:<github_url>`) | The durable per-repo scan **job**. Conditionally resolves the repo ref (`app/scan/fetch.py::resolve_ref` sends `If-None-Match`/`If-Modified-Since` from `repo_fetch_state` — a 304 is free), then: **unchanged ref + current rubric+engine** → skip (bump `last_checked_at`); **rubric/engine bump only (content same)** → re-evaluate from STORED `artifact_blobs` (no GitHub re-crawl, `source='rescan_rules'`); **content changed / never scanned** → full fetch + scan (`source='ingestion'`). **Size-gated hybrid fetch:** a repo whose reported `size` exceeds `SCAN_LARGE_REPO_SIZE_KB` (~20 MiB) skips the 25 MiB-capped tarball and uses `engine.run_repo_scan_via_trees` (Git Trees API — 1 REST call pinned to HEAD SHA — + per-blob `raw.githubusercontent.com` fetch of every file ≤5 MiB, fan-out bounded by `SCAN_TREES_FETCH_CONCURRENCY`); a smaller repo uses `engine.run_repo_scan` (tarball). **The in-memory file index is bounded on BOTH paths** by `SCAN_MAX_INDEX_FILES` / `SCAN_MAX_INDEX_TOTAL_BYTES` (the shared `fetch.IndexBudget`), applied in **sorted-path order**, so the kept fileset is identical regardless of fetch path — the 25 MiB tarball cap is compressed-stream-only, so the tarball/walk path previously loaded the WHOLE uncompressed repo into RAM (the OOM driver); over-budget files are skipped + reported. Because `size` is approximate, a tarball that blows the cap raises `TarballTooLargeError` (a `FetchError` subclass) and `_full_scan` retries once via the trees path before marking the run failed; a truncated tree / deleted repo / invalid ref stays a plain `FetchError` → run `failed` + recency stamped. Both paths converge on `_score_file_index` → `persist_completed_scan_run` with the identical fileset, so scores/snapshot/`.zip`/`content_hash_sha256` are byte-identical regardless of fetch path. Persists + stamps `last_scanned_at`/`scanned_*_version` on every public-github cap of the repo. Idempotent on `(github_url, ref_sha, rubric_version)`. Bounded by `SCAN_MAX_CONCURRENCY` (in-body semaphore). Pure decision helper: `decide_scan_action`. |
| `auto_scan_reconcile` (periodic, every 10 min, `queueing_lock`) | The coverage + versioned-re-eval + freshness **drainer**. Selects public-github not-archived repos with a resolved `github_url` (`github_url IS NOT NULL`) — **ANY quality tier** (the `quality_tier IN ('high','medium')` gate was removed so every indexed, repo-resolved capability is scanned, matching the merger's tier-less on-add scan hook; only repo-less rows — uploads / unresolved registry listings — stay excluded since there is nothing to fetch) — that are `last_scanned_at IS NULL` (coverage) OR `scanned_rubric_version`/`scanned_engine_version` ≠ current (rule/engine bump) OR `last_checked_at < now() - SCAN_FRESHNESS_DAYS` (freshness), **popularity-first**, deduped per repo URL, bounded by `SCAN_RECONCILE_BATCH`, and `defer_scan_job`s each (the `queueing_lock` dedups against in-flight jobs). **Back-pressure:** before selecting, `run_reconcile` reads the `scan`-queue backlog (`scan_backlog_count` — a cheap `count(*)` of `todo` `scan` jobs in `procrastinate_jobs`, guarded by `to_regclass`) and SKIPS the entire tick when it is at/above `SCAN_RECONCILE_MAX_BACKLOG` (default 500; staging 200) — never deepening the queue or the shared-PG pressure on a worker already behind; the 10-min cadence re-checks next tick. Testable inner: `run_reconcile` (the `backlog` counter is injected so tests simulate queue depth without the procrastinate schema). Gated off by `SCAN_AUTOSCAN_ENABLED=false`. |
| `recover_orphaned_jobs_at_boot` (worker boot, `tasks_scan.py`) | **Immediate** orphan recovery, called from `app.worker_main` after the connector opens + before the supervisor. At boot the previous worker is dead, so every `doing` job is an orphan whose `queueing_lock` blocks fresh cycles — it sweeps ALL queues (`scan` + ingest + `periodic`) at `nb_seconds=0` and re-queues them. Without it a deploy/restart orphan blocked its source for up to `INGESTION_STALLED_SECONDS` (4h) — the staging-ingestion-stalls-after-every-worker-redeploy bug. Shares the resilient `_requeue_stalled_jobs` helper with the periodic retriers. |
| `scan_stalled_retrier` (periodic, every 15 min) | Re-queues `scan`-queue jobs the worker abandoned mid-run (Procrastinate `get_stalled_jobs` + `retry_job`, `nb_seconds=_STALLED_SECONDS` 30 min). Steady-state net for restarts between boots. |
| `ingestion_stalled_retrier` (periodic, every 15 min) | Sibling to `scan_stalled_retrier` for the **ingest** queues (`ingest_github`/`ingest_aggregator`/`ingest_mcp_registry`/`ingest_npm`/`ingest_pypi`/`periodic`) — `scan_stalled_retrier` is `queue="scan"` only, so without this a worker restart orphaned a `doing` ingest cycle forever (the ~1 orphaned cycle/hour leak). Uses a generous `INGESTION_STALLED_SECONDS` (default 4h, comfortably above mcp_registry's worst-case cycle) so a long in-flight crawl is never re-queued out from under itself. **All three share `_requeue_stalled_jobs`, which is resilient PER JOB + PER QUEUE** — a single `retry_job` `queueing_lock` `UniqueViolation` (a stalled job whose lock a sibling `todo` already holds) is logged + skipped (self-heals next tick), never aborting the whole sweep (the crash that previously left every other orphan stuck). |
| `ingestion_run_reaper` (periodic, every 15 min; `app/ingestion/tasks.py`) | Reaps orphaned `ingestion_runs` **records** a reload left in `status='running'` (a restart between `record_run_started`/`record_run_finished`) — `mark_stale_ingestion_runs` flips every `running` row older than `INGESTION_STALLED_SECONDS` to `failed`/`StaleRunReaped`. **Distinct from `ingestion_stalled_retrier`**: that re-queues a stalled *job*; this reaps a *run-record* (which has no job to retry). Also runs at boot (`app/main.py`, after `recover_stale_scans`). Without it the eagle-eye view shows phantom `stuck`/inflated-`running`. See `database.md` § Ingestion runs. |
| `ingestion_overdue_reconciler` (periodic, every 15 min; `app/ingestion/tasks.py`) | The **missed-cron safety net** — the ingestion mirror of `auto_scan_reconcile`. Procrastinate's periodic deferrer only backfills a *recent* tick on worker boot, so a daily-cron source (`github_topics`, `pypi`, every aggregator) whose `0 1 * * *`-style tick fired hours before a **frequently-restarting** worker last booted is **never deferred** — the hourly sources (`mcp_registry`/`npm`) stay healthy while the daily ones silently never run (the staging "skills frozen / github_topics dead 8 days" bug). `reconcile_overdue_sources` re-defers any enabled `api`/`scrape` source whose latest cron tick has not been attempted — "last attempt" = `max(ingestion_runs.started_at)`, written at cycle START for **every** attempt (`record_run_started`, success **or** fail), so a consistently-failing source is re-fired once per scheduled tick, never every 15 min. The per-source `ingest_cycle_<source>_lock` `queueing_lock` (via `defer_source_cycle`, the same lock-aware pattern as `tasks_scan.defer_scan_job` + admin force-cycle) dedups against the real cron + an in-flight cycle, so it can't double-fire; paused/blocked/disabled sources are skipped (`is_source_paused`). `reconcile_overdue_sources` is the testable injected-deferrer entry point. |
...
`POST/GET /api/v1/admin/*` (`app/routers/admin.py`) are gated by the `X-Admin-Key` header (`SAFERSKILLS_ADMIN_KEY`; **fails closed** — every endpoint 403s when the secret is unset, **except** local development where `ENV=development` with no key configured is exempt and audits as the `local-dev` fingerprint; staging/production always set `ENV` so the exemption never applies on a deploy). Every mutation writes one `admin_audit_log` row (`security.md` § Audit Trail). Surface: `sources` (list / runs / pause / unpause / force-cycle), `merge-candidates` (list / decide), `catalog` (re-classify / inspect-events / archive / un-archive), `popularity` (recompute-now / top-n). **`force-cycle` is lock-aware** (mirrors `tasks_scan.defer_scan_job`): it pre-checks the per-source `queueing_lock` and returns **409** `"a cycle is already queued or running"` when a cycle is already `todo`/`doing` (catching `AlreadyEnqueued` as the race backstop) — an honest "busy", NOT the old broad-except "not schedulable" mislabel; a webhook/disabled source (no registered `ingest_cycle_*` task → `TaskNotFound`) returns **400** `"… has no periodic cycle (webhook or disabled source)"`; any other error is a real **500** (not swallowed). The TUI's force-cycle-ALL (`c`) tallies a 409 as **acknowledged-with-note** (the source is cycling, not failed), so an already-running source is never counted as a failure. `re-classify` re-runs the deterministic classifier on stored signals (no network re-fetch — `metadata_files` are reconstructed from the stored `kind_signals` so file-derived kind/agent classifications are preserved).
...
| Scrape fetch policy / tier / Cloudflare handling change | "Scrape fetch policy" + `framework/scraping_adapter.py` + `tasks.run_source_cycle` blocked path + `contributor-docs/sources.md` |
...
| Conflict resolution rule change | "Conflict resolution" — needs a documented exception |
...
| Popularity formula / weights change | "Periodic tasks" — bump `popularity_formulas` (new `active` row + migration) + `app/ingestion/popularity.py` |
...
| Archive timeline / alert-tier threshold change | "Periodic tasks" — `tasks_archive.py` / `framework/alerts.py` |
| Alert platform-label / cooldown / signature change | "Periodic tasks" (Alerts) — `framework/alerts.py` (`evaluate_alerts`) + `crawler_cursors.{last_alerted_at,alert_signature}` (migration 0026, `database.md`) + `INGESTION_ALERT_COOLDOWN_S` (`environment-config.md`) + `slack_invite_health.py` (`[env]` prefix) |
...
| Force-cycle error-classification change | "Admin endpoints + CLI" (409/400/500) + `app/routers/admin.py::admin_force_cycle_source` + the TUI `_confirm_force_cycle_all` 409 tally |
| Stale-running-run reaper change | "Durable auto-scan pipeline" (`ingestion_run_reaper` row) + `app/ingestion/tasks.py` + `app/main.py` boot hook + `database.md` § Ingestion runs |
| Overdue-cycle reconciler change (missed-cron net) | "Durable auto-scan pipeline" (`ingestion_overdue_reconciler` row) + `app/ingestion/tasks.py` (`reconcile_overdue_sources` / `defer_source_cycle`) + `tests/test_ingestion/test_overdue_reconciler.py` |
