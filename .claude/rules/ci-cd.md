...
| 19 | `cli-rustls` | `cargo tree -e normal` must contain no `openssl-sys` / `native-tls` — the rustls-only invariant (I-05). The I-5.5 `scan agent` deps (`ed25519-dalek` verify-only, `base64`) are pure-Rust and pull neither, so the lane stays green |
...
- **Founder action (one-time, outbox/01)**: the 6 npm TP bindings + the crates.io TP binding → `publish-npm.yml`; `npm deprecate @openlatch/saferskills`. Provenance needs a public repo. The observability secrets (`POSTHOG_PROJECT_KEY`, `SENTRY_DSN_CLI`, `SENTRY_AUTH_TOKEN`) are repo-level GH Actions secrets.
