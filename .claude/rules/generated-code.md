...
- `services/api/app/generated/rule_content.json` — snake_case backend mirror of the rule-content map. The server-side **prose join source**: `app/services/rule_prose.py` loads it and `app/scan/report_builder.py` inlines each fired rule's title/explanation/remediation onto the report finding (D-05-32 reversed — there is **no** `/rubric/content` endpoint and the CLI carries **no** rule corpus; it renders straight from the report it already has). Emitted by `generate-methodology.cjs`.
