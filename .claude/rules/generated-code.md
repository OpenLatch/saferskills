...
- `webapp/src/generated/methodology/index.mdx` + `rule-count.ts` and `webapp/src/generated/rules/content.ts` — methodology MDX + the `RULE_CONTENT` explainable-finding map (from `rubric/**/*.md` frontmatter, via `generate-methodology.cjs`)
- `services/api/app/generated/rule_content.json` — snake_case backend mirror of the rule-content map. The server-side **prose join source**: `app/services/rule_prose.py` loads it and `app/scan/report_builder.py` inlines each fired rule's title/explanation/remediation onto the report finding (there is **no** `/rubric/content` endpoint and the CLI carries **no** rule corpus; it renders straight from the report it already has). Emitted by `generate-methodology.cjs`.
...
| Change | Updates here |
|---|---|
| New generated output path added | "Generated paths" list + `schema-driven-development.md` |
