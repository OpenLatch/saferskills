...
| Agent-directory surface change (I-5.6 Phase C) | `database.md` § Agent directory surface (migration 0020 + `agent_verify_waitlist` + `agent_runs.vendor_reply`) + `privacy.md` § agent_verify_waitlist + `environment-config.md` (`AGENT_CORPUS_GATE_N`) + `design-system.md` § Agent directory vocabulary + root `CLAUDE.md` (webapp map) + the page/island/components under `webapp/src/pages/agents/` + `webapp/src/components/agent/` + `ui/components/*` |
