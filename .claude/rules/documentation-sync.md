...
The rules live under `.claude/rules/`.
...
| | `methodology.md` (`contributor-docs/methodology.md`, `contributor-docs/rules.md`, `rubric/**`) |
| | `vendor-appeals.md` (vendor-appeal templates + flows) |
| | `database.md` (`services/api/migrations/**`, `app/models/**`, `app/db/**`) |
| | `ingestion.md` (`services/api/app/ingestion/**`, `schemas/ingestion-*.schema.json`, `schemas/merge-candidate.schema.json`, `tools/saferskills-admin/**`, `contributor-docs/sources.md`) |

## Deferred rules (write when the feature lands)

| Rule | Lands with |
|---|---|
| `security-auth.md` | later (with auth) — auth, magic links, account routes |
| ~~`database.md`~~ | **Written** (migration `0006_add_artifact_blobs` — first content-storage subsystem). Now active above. |
| `multi-tenancy.md` | NEVER — SaferSkills is single-tenant public. Only write if the product pivots to multi-tenant. |
| `client-schemas-sync.md` | If a cross-repo wire-format package is published (no plans) |
...
| ~~`ingestion.md`~~ | **Written** (the ingestion framework + adapter rules). Now active above. |
| ~~`privacy.md`~~ | **Written** (the `access_log` writer + IP-redaction gate). Now active (always-on) above. |
...
|-------------|---------------|
| **New or changed user-facing feature** — an API endpoint, CLI command, webapp page, or any customer-observable behavior | The matching page under `webapp/src/content/docs/<section>/` (the public `/docs` site) — created or updated in the SAME PR. A brand-new top-level section also needs a `{dir,label}` in `webapp/src/lib/docs/sections.ts` + its mirror in `webapp/scripts/generate-llms-txt.cjs::SECTIONS`. |
...
| Agent-scan store/migration change | `database.md` § Agent scan + the model under `app/models/` + migration 0019 + `privacy.md` (telemetry) + `security.md` (no-raw-payload trace) |
| Agent-scan bootstrap / `agent` CLI change | `cli/README.md` (`agent` row) + root `CLAUDE.md` (CLI line) + `app/agent_scan/bootstrap/*.md` templates + `security.md` § Scan-trace transparency (pre-flight verify) + `fixtures/agent-scan-report.sample.json` (if the wire DTO shape changes) |
| Agent-directory surface change | `database.md` § Agent directory surface (migration 0020 + `agent_runs.vendor_reply`) + `environment-config.md` (`AGENT_CORPUS_GATE_N`) + `design-system.md` § Agent directory vocabulary + root `CLAUDE.md` (webapp map) + the page/island/components under `webapp/src/pages/agents/` + `webapp/src/components/agent/` + `ui/components/*` |
...
| New explainable-finding rule field | `schemas/rubric-rule.schema.json` + `methodology.md` + `contributor-docs/methodology.md` + `generate-methodology.cjs` (emit into `content.ts`) + every rubric doc backfilled in the same PR |
...
6. **Public docs vs contributor docs are in sync** — `contributor-docs/methodology.md` (public-facing) and `.claude/rules/methodology.md` (contributor-facing) cover the same contract; never let them drift.
7. **Customer docs ship with the feature.** Any change a customer can observe — a new or changed API endpoint, CLI command, webapp page, or behavior — MUST update the public `/docs` site (`webapp/src/content/docs/**`) in the SAME PR. The `/docs` site is the canonical user-facing reference; a feature that ships without its docs update is an incomplete PR.

## When to update this rule

| Change | Updates here |
|---|---|
| New rule file shipped | "Rule files" table — move from deferred to active if applicable |
