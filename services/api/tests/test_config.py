...
def test_turnstile_secret_present_passes_in_prod(env: EnvTier) -> None:
    ...
    settings = Settings(
        env=env, turnstile_secret_key="1x000...AA", cli_pow_secret="prod-pow-secret"
    )
    assert settings.turnstile_secret_key == "1x000...AA"
    assert settings.cli_pow_secret == "prod-pow-secret"


def test_cli_pow_secret_optional_in_dev() -> None:
    """Dev/test tolerate a missing CLI PoW secret — /cli-challenge 503s there."""
    settings = Settings(env="development", cli_pow_secret=None)
    assert settings.cli_pow_secret is None
...
def test_cli_pow_secret_required_in_prod(env: EnvTier) -> None:
    ...
    with pytest.raises(ValueError, match="CLI_POW_SECRET"):
        Settings(env=env, turnstile_secret_key="1x000...AA", cli_pow_secret=None)
