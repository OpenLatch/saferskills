...
class Settings(BaseSettings):
    ...
    # PoW challenge replaces Turnstile for CLI scan-submit. `cli_pow_secret` is the
    ...
    cli_pow_secret: str | None = Field(
        default=None,
        description=(
            "HMAC-SHA256 secret signing the stateless CLI Proof-of-Work challenge. "
            "None → /cli-challenge 503 + verify_pow rejects (dev/test/CI fall back "
            "to Turnstile). Required in staging/production (boot hard-fails otherwise)."
        ),
    )
    ...
    def _require_turnstile_secret_in_prod(self) -> Settings:
        ...
        if self.env in ("staging", "production") and self.cli_pow_secret is None:
            raise ValueError(
                "CLI_POW_SECRET is required in staging/production — it is the only "
                "trust anchor of the stateless CLI Proof-of-Work gate (must be a "
                "stable Fly secret, identical across canary machines)."
            )
        return self
