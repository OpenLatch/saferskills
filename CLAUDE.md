...
webapp/            # Astro 6 + React 19 public catalog (placeholder W1, real W3). Agent Report at /agents/[id] + /agents/r/[token] + the /agents directory (I-5.6 Phase C — corpus risk meter + dossier grid + infinite scroll), the agent badge /badge/agent/[id]/[score].svg, and the /methodology Agent-pack section (mirrors /scans; API stays /api/v1/agent-scans/* incl. GET list + GET aggregate-stats + r/{token}/reply). The I-06 Starlight **docs site** lives in the SAME package as a SECOND Astro config (`docs.astro.config.mjs`, SSG, `srcDir ./docs`, `base /docs`) — content under `webapp/docs/content/docs/`, brand-themed, served at `/docs/*` as a static fall-through on the Node server (`build:docs` folds `dist-docs` into `dist/client/docs`). Plan 1 = scaffold + IA placeholders; Plans 2/3 author + polish.
