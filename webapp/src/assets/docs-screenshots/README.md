...
docs via the `Screenshot.astro` component (`webapp/src/components/docs/Screenshot.astro`),
...
- **Not a CI lane.** Capture needs a running deployment and is slow; it is an
  on-demand refresh. The `docs-build` (frontmatter) + `lighthouse-a11y` (docs axe
  + link check) lanes gate the docs — every embed carries descriptive `alt` text
  to stay axe-clean.
...
- Captured at a pinned 1440×900 viewport, `deviceScaleFactor: 2`. Served via the
  Sharp-free passthrough image service (the build runs on Alpine), so the PNGs
  ship as-is — keep them reasonably sized.
