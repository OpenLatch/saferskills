---
title: "Install in Windsurf"
description: "Where Windsurf loads installed capabilities from, the path SaferSkills targets, and how to install with the trust score checked first."
updated: 2026-06-16
---
SaferSkills installs into Windsurf by writing under `~/.windsurf/`, which is both the load path and the config location in the agent manifest. The `saferskills install` command detects Windsurf automatically, shows the capability's trust score and five-axis breakdown, discloses that it is writing to Windsurf, and gates on the aggregate score before anything lands on disk. You can also install by hand from the public catalog.

## Where does Windsurf load installed capabilities from?

Windsurf loads from `~/.windsurf/`, and the same directory is the config location SaferSkills records against. SaferSkills writes a capability into that path on install and reverses exactly what it wrote on uninstall.

| Setting | Value |
| --- | --- |
| Install path | `~/.windsurf/` |
| Config file | `~/.windsurf/` |

This path is the agent manifest SaferSkills ships with. SaferSkills only manages the capabilities it placed there — it does not modify the rest of your Windsurf configuration.

## How do I install with the CLI?

Run `install` with the catalog name. The CLI resolves the name to a catalog item, prints a digest, discloses that it is writing to Windsurf, and then applies the install score gate.
...
`uninstall` reverses exactly what the CLI wrote under `~/.windsurf/`:
