...
<Screenshot
  name="scan-report"
  alt="A repo scan report: a consolidated 0–100 score with its color band and per-scan facts, above the list of discovered capabilities and their individual scores."
/>

A repository fans out into one report per discovered capability. Opening a single capability shows its own item report — the aggregate score, the latest-scan facts, install activity, and the per-category score breakdown:

<Screenshot
  name="item-report"
  alt="A capability item report: the aggregate score and band, the latest-scan facts, SaferSkills install activity, and the per-category score-breakdown table."
/>
